...
DoublyLinkedList::DoublyLinkedList(Node* front) {
  m_front = front;
  m_size = 0;
  m_back = nullptr;
...
DoublyLinkedList* DoublyLinkedList::getPositionList(DoublyLinkedList* elements) {
  Node* curElement = elements->getFront();
  int elementVal = 0;

  Node* curIndex = new Node();
  Node* frontNewList = curIndex;

  bool firstElement = true;

  while (curElement != nullptr) {
    Node* newNode = new Node();
    curIndex->setNext(newNode);

    //first element will not have a previous pointer
    if (firstElement) {
        firstElement = false;
    } else {
      newNode->setPrev(curIndex);
    }

    //set new list's value at each node to index of found value
    //-1 if not found
    elementVal = curElement->getValue();
    curIndex->setValue(this->find(elementVal));
  ...
  //delete last node that does not correspond to any meaningful data
  curIndex->getPrev()->setNext(nullptr);
  delete curIndex;

  return new DoublyLinkedList(frontNewList);
}
