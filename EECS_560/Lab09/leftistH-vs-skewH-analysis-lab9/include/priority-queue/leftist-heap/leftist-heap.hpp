...
public:
   ...
   */
  LeftistHeap(Task** tasks, int taskSize);
  ...
  /**
   * Adds one task element to LeftistHeap and restructures the heap to
   ...
   */
  int inorderTraversal(node_t* treePtr, int identity, int (LeftistHeap::*op)(Task*));

  /**
   * This function performs a post traversal over the elements of
   * the priority queue and applies the operation *op on each of the elements.
   */
  int postorderTraversal(node_t* treePtr, int identity, int (LeftistHeap::*op)(Task*));

  /**
   * This function performs a pre traversal over the elements of
   * the priority queue and applies the operation *op on each of the elements.
   */
  int preorderTraversal(node_t* treePtr, int identity, int (LeftistHeap::*op)(Task*));

  int getSize();

  node_t* getRootPtr();

  int sizeTraversal(node_t *treePtr, int identity, int (LeftistHeap::*op)(int));
